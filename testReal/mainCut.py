...
def reserveList(fL,l):
    ...
    for i in l:
        f.append(fL[i])
    return f
def testOnce(cutdownX = True,xPer = 0.85 * 100,dataType='default',domainCorrlation=False,method='ALL',minmax=False):
    ...
    seq, rate, res1, res2, res3, (f1, f2, f3) = pipLineTestUnet(data, thresRes, epoch=3, ms=ms, name='DEAndADE',
                                                                ...
                                                                cutdownX=cutdownX,xPer=xPer,domainCorrlation=domainCorrlation)
    # if cutdownX:
...
if __name__ == '__main__':
    ...
    # testOnce(minmax=True,cutdownX = True,xPer = 0.85 * 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    # testOnce(minmax=True,cutdownX = True,xPer = 0.875 * 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    # testOnce(minmax=True,cutdownX = True,xPer = 0.90 * 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    # testOnce(minmax=True,cutdownX = True,xPer = 0.925 * 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    # testOnce(minmax=True,cutdownX = True,xPer = 0.95* 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    # testOnce(minmax=True,cutdownX = True,xPer = 0.975* 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    testOnce(minmax=True,cutdownX = False,xPer = 0.975* 100,dataType='Ndefault',method='UAS',domainCorrlation=True)
    # res['res'][6, i, 2, 2] = MAE(x3P, dT[:, 1])
