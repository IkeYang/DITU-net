...
dirP=r"D:\YANG Luoxiao\Data\WPC\Generate\SecondDEOnly"
# saveFig = r'D:\YANG Luoxiao\Data\WPC\Generate\FirstDEOnly\testRes'
saveFig = r'D:\YANG Luoxiao\Data\WPC\Generate\SecondDEOnly\testRes'
transf = transforms.Compose(
    [
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
trainDatas=WPCDEDataset(2000,dirP+r'\\'+'train',transform=transf)
valDatas=WPCDEDataset(400,dirP+r'\\'+'test',transform=transf)
...
for epoch in range(start_epoch, start_epoch + epochs):
    ...
    with torch.no_grad():
        ...
        for j in range(3):
            ...
            imgx = transform_invert(y[j, :].detach().cpu(), transf)
            axs[j, 2].imshow(imgx)
        f.savefig(saveFig + r'\\' + 'epoch%dRes%d.jpg' % (epoch, 0000))
        plt.close(f)
    ...
    torch.save(state, '%s/UnetS2000DE.pth' % (outf))
